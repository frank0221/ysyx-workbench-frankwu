...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = (uint32_t)(Mr(src1 + imm, 2) & 0xffff));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1 + imm, 2),16));//读半字的时候要符号扩展
  ...
  // INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = (uint32_t) (src1) >> (imm & 00000000000000000000000000011111));
  ...
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, s->dnpc = ((uint32_t)src1 < (uint32_t)src2) ? s->pc + imm :  s->snpc);
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = (int32_t)src2 == 0 ? -1 :
                                                                        (int32_t)src1 == INT32_MIN && (int32_t)src2 == -1 ? INT32_MIN:
                                                                        (int32_t)src1 / (int32_t)src2);
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = src2 == 0 ? UINT32_MAX :
                                                                        src1 / src2);                                                                      
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = (int32_t)src2 == 0 ? src1 :
                                                                        ((int32_t)src1 == INT32_MIN && (int32_t)src2 == -1) ? 0 :
                                                                        (int32_t)src1 % (int32_t)src2);
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(rd) = (src2 == 0 ? src1 : src1 % src2));
                                                                        
#ifdef CONFIG_FTRACE
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc, s->dnpc = (src1 + imm )& ~1,ftrace(rd ,(int)BITS(s->isa.inst, 19, 15) ,s->dnpc, s->pc));
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc, s->dnpc = s->pc + imm, ftrace(rd ,(int)BITS(s->isa.inst, 19, 15) ,s->dnpc, s->pc));//, printf("jar pc at 0x%x\n",s->pc));
#else
