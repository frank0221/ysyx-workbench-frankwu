...
module PSRAM_READER (
    ...
    always @ (posedge clk)
        if(counter >= 20-6 && counter <= FINAL_COUNT-6)
            if(sck)
                data[byte_index] <= {data[byte_index][3:0], din}; // Optimize!

    assign dout     =   (counter < 8-6)   ?   {CMD_EBH[7-4*counter -: 4]}:
    ...
    assign done     = (counter == FINAL_COUNT-6+1);
...
module PSRAM_WRITER (
    ...
    assign dout     =   (counter < 8-6)   ?   {CMD_38H[7-4*counter -: 4]}:
                        (counter == 8-6)  ?   saddr[23:20]        :
                        (counter == 9-6)  ?   saddr[19:16]        :
                        (counter == 10-6) ?   saddr[15:12]        :
                        (counter == 11-6) ?   saddr[11:8]         :
