...
void init_disasm() {
    fprintf(stderr, "[init_disasm] entered\n");
    fflush(stderr);        // 确保立即刷出
  void *dl_handle;
  //dl_handle = dlopen("tools/capstone/repo/libcapstone.so.5", RTLD_LAZY);
  dl_handle = dlopen("/home/frank_wu/ysyx-workbench/nemu/tools/capstone/repo/libcapstone.so.5", RTLD_LAZY);
  if (!dl_handle) {
  ...
  fprintf(stderr,
        "[init_disasm] cs_open_dl=%p, cs_disasm_dl=%p, cs_free_dl=%p\n",
        (void*)cs_open_dl, (void*)cs_disasm_dl, (void*)cs_free_dl);
